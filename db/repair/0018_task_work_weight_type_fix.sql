-- TASK-FIX-03 — chuẩn hóa kiểu tham số khi gọi public.can_set_work_weight(uuid, smallint).
-- Lỗi: trong task_recurrence_create, COALESCE(_work_weight, 2) trả về integer (hằng 2 là int4),
-- nên PostgreSQL tìm can_set_work_weight(uuid, integer) và báo "does not exist".
-- Sửa: ép hằng mặc định về smallint. Không đổi Business Rule (1/2/3/5, mặc định 2, chỉ Leader/CMO/Admin đặt 5).
...
-- Chỉ giữ đúng một signature chuẩn (uuid, smallint): dọn overload (uuid, integer) nếu tồn tại.
